...
BEGIN
  INSERT INTO payments(id, booking_id, amount, payment_method, status, notes)
  VALUES (v_payment_id, p_booking_id, p_amount, p_payment_method, 'paid', p_notes);

  INSERT INTO journal_entries(id, booking_id, entry_type, description, total_amount, reference_number)
  VALUES (v_journal_id, p_booking_id, 'PAYMENT_RECEIVED', COALESCE(p_notes, 'Payment received'), p_amount, 'PMT-' || v_payment_id::TEXT);
...
BEGIN
  ...
  -- Try to get booking information if booking_id exists
  ...
  -- Verify journal entry was created successfully
  ...
  -- If full payment, add revenue recognition entries
  ...
  -- If full payment, add revenue recognition to general ledger
  IF v_is_full_payment THEN
    INSERT INTO general_ledger (
      transaction_date, 
      booking_id, 
...
-- Update the journal entries constraint to include payment types
...
-- Create a function to check and fix missing journal entries for existing bookings

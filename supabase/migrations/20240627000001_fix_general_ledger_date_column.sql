-- Fix the general_ledger date column issue
...
-- Create function to handle booking journal entries with proper date values for both tables
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    ...
    -- Insert journal entry header with both date and entry_date
    INSERT INTO journal_entries (
      date, entry_date, booking_id, entry_type, description, total_amount, reference_number
    ) VALUES (
      v_current_date, v_current_date, NEW.id, v_entry_type, v_description, NEW.total_amount, v_reference_number
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, NEW.id, v_ar_account_id, v_description, NEW.total_amount, 0, v_reference_number
    );
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, NEW.id, v_unearned_revenue_id, v_description, 0, NEW.total_amount, v_reference_number
    );
  ...
  ELSIF TG_OP = 'UPDATE' AND OLD.payment_status = 'unpaid' AND NEW.payment_status = 'paid' THEN
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, NEW.id, v_cash_account_id, v_description, NEW.total_amount, 0, v_reference_number
    );
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, NEW.id, v_ar_account_id, v_description, 0, NEW.total_amount, v_reference_number
    );
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, NEW.id, v_unearned_revenue_id, v_description, NEW.total_amount, 0, v_reference_number
    );
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, NEW.id, v_revenue_account_id, v_description, 0, NEW.total_amount, v_reference_number
    );
  END IF;
...
-- Create function to handle payment journal entries with proper date values for both tables
...
BEGIN
  ...
  -- Insert journal entry header with both date and entry_date
  INSERT INTO journal_entries (
    date, entry_date, booking_id, payment_id, entry_type, description, total_amount, reference_number
  ) VALUES (
    v_current_date, v_current_date, v_booking_id, NEW.id, v_entry_type, v_description, v_payment_amount, v_reference_number
  ) RETURNING id INTO v_journal_entry_id;
  
  -- Insert general ledger entries
  -- Debit Cash
  INSERT INTO general_ledger (
    date, transaction_date, booking_id, payment_id, account_id, description, debit, credit, reference_number
  ) VALUES (
    v_current_date, v_current_date, v_booking_id, NEW.id, v_cash_account_id, v_description, v_payment_amount, 0, v_reference_number
  );
  ...
  INSERT INTO general_ledger (
    ...
    v_current_date, v_current_date, v_booking_id, NEW.id, v_ar_account_id, v_description, 0, v_payment_amount, v_reference_number
  );
  ...
  IF v_is_full_payment THEN
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, v_booking_id, NEW.id, v_unearned_revenue_id, v_description, v_total_amount, 0, v_reference_number
    );
    ...
    INSERT INTO general_ledger (
      ...
      v_current_date, v_current_date, v_booking_id, NEW.id, v_revenue_account_id, v_description, 0, v_total_amount, v_reference_number
    );
  END IF;
